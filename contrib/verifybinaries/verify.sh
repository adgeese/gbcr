...
###   goldbcrcore.org and goldbcr.org and compares them.
...
WORKINGDIR="/tmp/goldbcr_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://goldbcrcore.org"
HOST2="https://goldbcr.org"
BASEDIR="/bin/"
VERSIONPREFIX="goldbcr-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'goldbcr-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "goldbcr.org failed to provide signature file, but goldbcrcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "goldbcr.org and goldbcrcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Gold BCR Core binary release signing key installed?"
   fi
