Gold BCR Core version 0.19.0.1 is now available from:

  <https://goldbcrcore.org/bin/goldbcr-core-0.19.0.1/>
...
Gold BCR Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Gold BCR Core on unsupported systems.
...
to open the Gold BCR Core .dmg. This is due to new signing requirements
imposed by Apple, which the Gold BCR Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/goldbcr/goldbcr/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Gold BCR Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Gold BCR
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `goldbcrd -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Gold BCR Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Gold BCR Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Gold BCR Core. (#14054)

- To eliminate well-known denial-of-service vectors in Gold BCR Core,
  ...
  This prevents Gold BCR Core from sending the BIP111 NODE_BLOOM service
  ...
  of Gold BCR Core and nodes that have manually enabled BIP37 support,
...
- By default, Gold BCR Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `goldbcr-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Gold BCR Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Gold BCR Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Gold BCR Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Gold BCR address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Gold BCR Core v0.17.0), this can eliminate a
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (goldbcrVBR)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/goldbcr/goldbcr/).
