...
Gold BCR Core version 0.19.0 is now available from:

  <https://goldbcrcore.org/bin/goldbcr-core-0.19.0/>
=======
0.20.0 Release Notes
====================

Gold BCR Core version 0.20.0 is now available from:

  <https://goldbcrcore.org/bin/goldbcr-core-0.20.0/>
...
installer (on Windows) or just copy over `/Applications/Gold BCR-Qt` (on Mac)
or `goldbcrd`/`goldbcr-qt` (on Linux).

Upgrading directly from a version of Gold BCR Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of Gold BCR Core are generally supported.

Compatibility
==============

Gold BCR Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Gold BCR
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Gold BCR Core on
unsupported systems.

From Gold BCR Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Gold BCR Core does not yet change appearance
...
#### Removal of BIP61 reject network messages from Gold BCR Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Gold BCR Core version 0.18.0.
...
- Testing or debugging of implementations of the Gold BCR P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Gold BCR Core. Gold BCR Core logs debug messages
...
- OpenSSL is no longer used by Gold BCR Core.  (#17265)

- BIP70 support has been fully removed from Gold BCR Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/goldbcr/goldbcr/issues/16599),
  [PR #16702](https://github.com/goldbcr/goldbcr/pull/16702), and the
  `goldbcrd help` for more information.  This option is experimental and
...
- All custom settings configured when Gold BCR Core starts are now
...
- The "Start Gold BCR Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://goldbcrcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Gold BCR Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Gold BCR Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/goldbcr-core/HWI) for
  signing.  Future versions of Gold BCR Core should support a GUI option
...
- Gold BCR Core's automatically-generated source code documentation is
  now available at https://doxygen.goldbcrcore.org.  (#17596)
...
- The `goldbcr-cli` utility used with the `-getinfo` parameter now
...
- The `goldbcr-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `goldbcrstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_goldbcr-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_goldbcr` binary (hebasto)
...
- #18702 Fix ASLR for goldbcr-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate goldbcr schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest goldbcr.conf (sdaftuar)
...
- #18022 Fix appveyor `test_goldbcr` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (goldbcr-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone goldbcr-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Gold BCR Core files (hebasto)
...
- #17119 Fix broken goldbcr-cli examples (andrewtoth)
...
- #17503 Remove bitness from goldbcr-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.goldbcrcore.org (laanwj)
...
- #17648 Rename wallet-tool references to goldbcr-wallet (hel-o)
...
- #16115 On goldbcrd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from goldbcr-cli -getinfo (malevolent)
...
- #15649 Add ChaCha20Poly1305@Gold BCR AEAD (jonasschnelli)
...
- #15639 goldbcr-wallet tool: Drop libgoldbcr_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in goldbcrd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "goldbcr-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of goldbcrd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from goldbcr_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_goldbcr to auto generated project files (sipsorcery)
...
- #15773 Add Gold BCRTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_goldbcr output (practicalswift)
...
- #15896 feature_filelock, interface_goldbcr_cli: Use PACKAGE_NAME in messages rather than hardcoding Gold BCR Core (luke-jr)
...
- #16277 Suppress output in test_goldbcr for expected errors (gertjaap)
...
- #16489 log: harmonize goldbcrd logging (jonatack)
...
- #15513 add "sections" info to example goldbcr.conf (fanquake)
...
- #16080 build/doc: update goldbcr_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in goldbcr.conf (fanquake)
...
- #16621 add default goldbcr.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the goldbcr-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, goldbcr.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/goldbcrd.cpp (ch4ot1c)
...
[Transifex](https://www.transifex.com/goldbcr/goldbcr/).
