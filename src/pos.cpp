...
bool CheckKernel(CBlockIndex* pindexPrev, unsigned int nBits, uint32_t nTimeBlock, const COutPoint& prevout, CCoinsViewCache& view, const std::map<COutPoint, CStakeCache>& cache)
{
    ...
    if(it == cache.end()) {
        ...
        // Check if coin is mature
        // if the block height is greater than COINBASE_MATURITY_LTS_HEIGHT set the WALLET_COIN_MINIMUM from 0 (pre-mining
        int64_t nCoinbaseMaturity = COINBASE_MATURITY;

        // Set the wallet minimum of the height is gt the COINBASE_MATURITY_LTS_HEIGHT
        if (pindexPrev->nHeight + 1 >= COINBASE_MATURITY_LTS_HEIGHT)
            nCoinbaseMaturity = COINBASE_MATURITY_LTS;

        if (coinPrev.nHeight >= COINBASE_MATURITY_LTS_HEIGHT &&
                pindexPrev->nHeight + 1 - coinPrev.nHeight < nCoinbaseMaturity){
