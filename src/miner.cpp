...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman, CTxMemPool* mempool)
{
    ...
    while (true)
    {
        ...
        // if the block height is greater than COINBASE_MATURITY_LTS_HEIGHT set the WALLET_COIN_MINIMUM from 0 (pre-mining
        int64_t nWalletMinimum = 0;

        // Set the wallet minimum of the height is gt the COINBASE_MATURITY_LTS_HEIGHT
        if (::ChainActive().Tip()->nHeight >= COINBASE_MATURITY_LTS_HEIGHT)
            nWalletMinimum = WALLET_COIN_MINIMUM;

        // Enable minimum
        if (setCoins.size() > nWalletMinimum)
        {
